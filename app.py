...
def demographics():
    ...
    with engine.connect() as connection:
        # Execute an SQL query to fetch data (replace this with your query)
        query2 = text('SELECT * FROM demographics')

        result2 = connection.execute(query2)

        # Fetch all rows of data
        demographicdata = result2.fetchall()
...
def patientpreferences():
    ...
    with engine.connect() as connection:
        # Execute an SQL query to fetch data (replace this with your query)
        query3 = text('SELECT * FROM patient_preferences')

        result3 = connection.execute(query3)

        # Fetch all rows of data
        preferencedata = result3.fetchall()

    return render_template('preferences.html', data3=preferencedata)
